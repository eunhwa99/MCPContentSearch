...
| PR CI retry investigation | completed | The next CI retry moved past the public-contract step and failed in the broader non-live suite because stale `tests/api/test_tools_contract.py` expectations still assumed public `answer_with_citations` existed and default `search_context` omitted `debug`. | `gh run view 27548056057 --log-failed`; `tests/api/test_tools_contract.py`; `.agents/docs/architecture.md` |
| Contract-test alignment | completed | Updated the API contract tests and fake search service so they match the maintained slim public tool surface: no public `answer_with_citations`, configured `search_context` keeps `debug={}` by default, and the `no_matching_sources` fast path still exposes debug. | `tests/api/test_tools_contract.py` |
| CI-like verification | completed | Re-ran the public/API contract subset and then the exact non-live CI pytest command on the merged branch state; both passed. | `uv run --locked pytest -q tests/api/test_tools_contract.py tests/contracts/test_public_mcp_contracts.py tests/test_app_composition.py` -> `44 passed`; `uv run --locked pytest -m "not live" --cov=api --cov=core --cov=environments --cov=fetching --cov=indexing --cov=search --cov=storage --cov-report=term-missing` -> `645 passed`; `git diff --check` |
