...
| GitHub live search parity | completed | Added `search_github` MCP tool and GitHub real-time search wiring through the legacy source-search path so configured repositories can be searched directly like Notion/Tistory before background indexing. | `fetching/github.py`; `fetching/web_searcher.py`; `api/tools.py`; `main.py`; `web_console/app.py`; `tests/api/test_tools_contract.py`; `tests/fetching/test_web_searcher.py` |
| Web console debug-first repositioning | completed | Reframed the browser UI as an indexed-evidence console: default mode is retrieval/debug, copy now explains synced-RAG-only behavior, and Codex mode is labeled as an experimental convenience summary rather than a Claude-equivalent answer surface. | `web/index.html`; `web/app.js`; `web/styles.css`; `README.md`; `docs/contextwiki-core-understanding.md`; `tests/web_console/test_app.py` |
