...
`search_context` now returns an additive `debug` block that includes rewrite
decision fields:
...
This makes it explicit whether rewrite was disabled, not needed, or attempted
but produced no applied rewrite.
